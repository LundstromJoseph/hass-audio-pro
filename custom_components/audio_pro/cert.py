"""mTLS certificates for Audio Pro C5 MkII / Arylic devices.

Sourced from pywiim (Apache 2.0):
https://github.com/mjcumming/pywiim/blob/main/pywiim/api/constants.py
Original cert from linkplay-cli / yamaha-soundbar (same shared vendor cert on all devices).
"""

import ssl
import tempfile
import os

# Shared CA cert — same on all LinkPlay/Audio Pro devices.
WIIM_CA_CERT = """-----BEGIN CERTIFICATE-----

uc2m8y8O/hXPSfKd
-----END CERTIFICATE-----"""

# Client cert (private key + cert) — shared vendor cert for mTLS on port 4443.
AUDIO_PRO_CLIENT_CERT = """-----BEGIN PRIVATE KEY-----

/NDvel7IF63U4UEkrVVt1bYg
-----END PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

uc2m8y8O/hXPSfKd
-----END CERTIFICATE-----"""


def create_ssl_context() -> ssl.SSLContext:
    """Return an SSLContext with the Audio Pro client cert loaded."""
    ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    ctx.check_hostname = False
    ctx.verify_mode = ssl.CERT_NONE
    # SECLEVEL=0 needed — the device uses an old cert that fails SECLEVEL≥1
    ctx.set_ciphers("DEFAULT:@SECLEVEL=0")

    # Write the combined PEM (key + cert) to a temp file so load_cert_chain can read it
    with tempfile.NamedTemporaryFile(mode="w", suffix=".pem", delete=False) as f:
        f.write(AUDIO_PRO_CLIENT_CERT)
        tmp_path = f.name

    try:
        ctx.load_cert_chain(tmp_path)
    finally:
        os.unlink(tmp_path)

    return ctx
